...
export let CreateJWTToken = (data: any = {}) => {
    ...
    const token = jwt.sign(tokenData, 'Lumel', { expiresIn: '10D' });
 ...
 * @param {Object} res 
 * @param {Function} next  
 * @description This Function is used to Check the session and Verify the token
 */
export let checkSession = async (req, res, next) => {
    ...
    if (token) {
        ...
        if (headerType.trim() === "Bearer") {
            try {
                jwt.verify(tokenValue, 'pixelpaisa', function (err, tokendata) {
                    if (err) {
